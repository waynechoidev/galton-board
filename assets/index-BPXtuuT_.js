...
}`;class D{constructor(e,n){h(this,"_label");h(this,"_device");h(this,"_pointBuffer");h(this,"_billboardBuffer");this._device=e,this._label=n}async initialize(e){const n=[];for(let o=0;o<e.length;++o){const{position:a,velocity:l,texCoord:d,isObstacle:P}=e[o];n.push(...a,...l,...d,P,0)}const u=new Float32Array(n);this._pointBuffer=this._device.createBuffer({label:`${this._label} point vertex buffer`,size:u.byteLength,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this._device.queue.writeBuffer(this._pointBuffer,0,u),this._billboardBuffer=this._device.createBuffer({label:`${this._label} billboard vertex buffer`,size:u.byteLength*6,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST})}get point(){return this._pointBuffer||console.error(`${this._label} vertex buffers should be initialized first!`),this._pointBuffer}get billboard(){return this._billboardBuffer||console.error(`${this._label} vertex buffers should be initialized first!`),this._billboardBuffer}}const le=r=>{const e=new Float32Array(r);for(let n=0;n<r;n++)e[n]=Math.random()<.5?-1:1;return e},ce=r=>r*(r+1)/2,C=document.documentElement.clientHeight*(document.documentElement.clientHeight>document.documentElement.clientWidth?.5:.7),b=C,U=256,c=22,I=ce(c),w=[.97,.92,.8],_=[.545,.271,.075],ue=async()=>{var q;const r=await((q=navigator.gpu)==null?void 0:q.requestAdapter()),e=await(r==null?void 0:r.requestDevice());if(!e){alert("need a browser that supports WebGPU");return}const n=document.querySelector("#board");n.style.width=`${b}px`,n.style.backgroundColor=`rgb(${w.map(t=>Math.round(t*255)).join(",")})`,n.style.border=`2px rgb(${_.map(t=>Math.round(t*255)).join(",")}) solid`;const u=C*.3,o=document.querySelector("#result");o.style.height=`${u}px`;const a=[];for(let t=0;t<c+1;t++){const i=document.createElement("span");i.className="slot",i.style.borderLeft=`0.5px rgb(${_.map(s=>Math.round(s*255)).join(",")}) solid`,t===c&&(i.style.borderRight=`0.5px rgb(${_.map(s=>Math.round(s*255)).join(",")}) solid`);const f=document.createElement("span");f.className="res",f.style.fontSize=`${b*.04}px`,i.append(f),a.push(f),o.append(i)}const l=document.querySelector("#funnel");l.style.borderTop=`${C*.05}px rgb(${_.map(t=>Math.round(t*255)).join(",")}) solid`,l.style.borderLeft=`${b*.1}px transparent solid`,l.style.borderRight=`${b*.1}px transparent solid`,l.style.width=`${b*.15}px`,l.style.marginTop=`${b*0}px`;const d=document.querySelector("canvas");d.style.width=`${b}px`,d.style.height=`${C}px`,d.width=b*2,d.height=C*2;const P=navigator.gpu.getPreferredCanvasFormat(),G=d.getContext("webgpu");G.configure({device:e,format:P});const T=[];for(let t=0;t<U;++t)T.push({position:[0,1+t*.01],velocity:[0,-1],texCoord:[0,0],isObstacle:0});const v=new D(e,"object");await v.initialize(T);const p=[],O=.088;for(let t=1;t<=c;++t)if(t%2===1){p.push({position:[0,.9-O*(t-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1});for(let i=0;i<Math.floor(t/2);i++)p.push({position:[0+.08*(i+1),.9-O*(t-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1}),p.push({position:[0-.08*(i+1),.9-O*(t-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1})}else for(let i=0;i<Math.floor(t/2);i++)p.push({position:[.04+.08*i,.9-O*(t-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1}),p.push({position:[-.04-.08*i,.9-O*(t-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1});const B=new D(e,"obstacle");await B.initialize(p);const j=e.createBuffer({label:"probabilities uniform buffer",size:U*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),V=e.createBuffer({label:"resultCriteria uniform buffer",size:c*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),H=p.slice(-c).map(t=>t.position[0]).sort((t,i)=>t-i);e.queue.writeBuffer(V,0,new Float32Array(H));const L=e.createBuffer({label:"result buffer",size:(c+1)*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),x=e.createBuffer({label:"result buffer",size:(c+1)*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST}),y=e.createBuffer({label:"constant uniform buffer",size:16*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});e.queue.writeBuffer(y,0,new Float32Array([1,0,0,.01,..._,.025,...w,c,I,0,0,0]));const M=e.createBuffer({label:"time uniform buffer",size:2*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),A=e.createComputePipeline({label:"compute movement pipeline",layout:"auto",compute:{module:e.createShaderModule({label:"movement compute shader",code:ie})}}),F=e.createComputePipeline({label:"compute result pipeline",layout:"auto",compute:{module:e.createShaderModule({label:"result compute shader",code:re})}}),E=e.createComputePipeline({label:"compute billboard pipeline",layout:"auto",compute:{module:e.createShaderModule({label:"billboard compute shader",code:ne})}}),Y=e.createRenderPipeline({label:"main pipeline",layout:"auto",vertex:{module:e.createShaderModule({label:"main vertex shader",code:ae}),buffers:[{arrayStride:8*Float32Array.BYTES_PER_ELEMENT,attributes:[{shaderLocation:0,offset:0,format:"float32x2"},{shaderLocation:1,offset:2*Float32Array.BYTES_PER_ELEMENT,format:"float32x2"},{shaderLocation:2,offset:4*Float32Array.BYTES_PER_ELEMENT,format:"float32x2"},{shaderLocation:3,offset:6*Float32Array.BYTES_PER_ELEMENT,format:"float32"}]}]},fragment:{module:e.createShaderModule({label:"main fragment shader",code:se}),targets:[{format:P}]},primitive:{topology:"triangle-list"}}),W=e.createBindGroup({label:"compute movement bind group",layout:A.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v.point}},{binding:1,resource:{buffer:B.point}},{binding:2,resource:{buffer:j}},{binding:3,resource:{buffer:y}},{binding:4,resource:{buffer:M}}]}),k=e.createBindGroup({label:"compute result bind group",layout:F.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v.point}},{binding:1,resource:{buffer:y}},{binding:2,resource:{buffer:V}},{binding:3,resource:{buffer:L}}]}),K=e.createBindGroup({label:"compute billboard bind group",layout:E.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v.point}},{binding:1,resource:{buffer:v.billboard}},{binding:2,resource:{buffer:y}}]}),X=e.createBindGroup({label:"compute obstacle billboard bind group",layout:E.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:B.point}},{binding:1,resource:{buffer:B.billboard}},{binding:2,resource:{buffer:y}}]}),J=e.createBindGroup({label:"main bind group",layout:Y.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:y}}]}),z=e.createCommandEncoder({label:"encoder"}),S=z.beginComputePass({label:"compute billboard pass"});S.setPipeline(E),S.setBindGroup(0,X),S.dispatchWorkgroups(1,1),S.end();const Q=z.finish();e.queue.submit([Q]);let N=0;async function $(){const t=performance.now(),i=t-N;N=t,e.queue.writeBuffer(M,0,new Float32Array([i])),e.queue.writeBuffer(j,0,le(U));const f=e.createCommandEncoder({label:"encoder"}),s=f.beginComputePass({label:"compute movement pass"});s.setPipeline(A),s.setBindGroup(0,W),s.dispatchWorkgroups(1,1),s.setPipeline(F),s.setBindGroup(0,k),s.dispatchWorkgroups(1,1),s.setPipeline(E),s.setBindGroup(0,K),s.dispatchWorkgroups(1,1),s.end();const g=f.beginRenderPass({label:"main pass",colorAttachments:[{view:G.getCurrentTexture().createView(),clearValue:[...w,1],loadOp:"clear",storeOp:"store"}]});g.setPipeline(Y),g.setBindGroup(0,J),g.setVertexBuffer(0,B.billboard),g.draw(I*6),g.setVertexBuffer(0,v.billboard),g.draw(U*6),g.end(),f.copyBufferToBuffer(L,0,x,0,x.size);const Z=f.finish();e.queue.submit([Z]),await x.mapAsync(GPUMapMode.READ);const R=new Float32Array(x.getMappedRange().slice(0));x.unmap();const ee=Math.max.apply(null,Array.from(R));for(let m=0;m<c+1;m++)a[m].style.height=`${R[m]/ee*u}px`,R[m]&&(a[m].innerHTML=R[m].toString());requestAnimationFrame($)}$()};ue();
