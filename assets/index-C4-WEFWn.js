...
}`;class ${constructor(e,o){y(this,"_label");y(this,"_device");y(this,"_pointBuffer");y(this,"_billboardBuffer");this._device=e,this._label=o}async initialize(e){const o=[];for(let n=0;n<e.length;++n){const{position:r,velocity:s,texCoord:b,isObstacle:d}=e[n];o.push(...r,...s,...b,d,0)}const a=new Float32Array(o);this._pointBuffer=this._device.createBuffer({label:`${this._label} point vertex buffer`,size:a.byteLength,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this._device.queue.writeBuffer(this._pointBuffer,0,a),this._billboardBuffer=this._device.createBuffer({label:`${this._label} billboard vertex buffer`,size:a.byteLength*6,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST})}get point(){return this._pointBuffer||console.error(`${this._label} vertex buffers should be initialized first!`),this._pointBuffer}get billboard(){return this._billboardBuffer||console.error(`${this._label} vertex buffers should be initialized first!`),this._billboardBuffer}}const le=t=>{const e=new Float32Array(t);for(let o=0;o<t;o++)e[o]=Math.random()<.5?-1:1;return e},ce=t=>t*(t+1)/2,x=Math.min(window.innerHeight*.7,window.innerWidth*.95),f=x,w=256,u=22,N=ce(u),q=[.97,.92,.8],O=[.545,.271,.075],R=document.querySelector("#board");R.style.width=`${f}px`;R.style.backgroundColor=`rgb(${q.map(t=>Math.round(t*255)).join(",")})`;R.style.border=`2px rgb(${O.map(t=>Math.round(t*255)).join(",")}) solid`;const D=x*.3,I=document.querySelector("#result");I.style.height=`${D}px`;const S=[];for(let t=0;t<u+1;t++){const e=document.createElement("span");e.className="slot",e.style.borderLeft=`0.5px rgb(${O.map(a=>Math.round(a*255)).join(",")}) solid`,t===u&&(e.style.borderRight=`0.5px rgb(${O.map(a=>Math.round(a*255)).join(",")}) solid`),e.style.width=`${f*.039}px`;const o=document.createElement("span");o.className="res",o.style.fontSize=`${f*.04}px`,e.append(o),S.push(o),I.append(e)}const B=document.querySelector("#funnel");B.style.borderTop=`${x*.05}px rgb(${O.map(t=>Math.round(t*255)).join(",")}) solid`;B.style.borderLeft=`${f*.1}px transparent solid`;B.style.borderRight=`${f*.1}px transparent solid`;B.style.width=`${f*.15}px`;B.style.marginTop=`${f*0}px`;const h=document.querySelector("canvas");h.style.width=`${f}px`;h.style.height=`${x}px`;h.width=f*5;h.height=x*5;const ue=async()=>{var z;const t=await((z=navigator.gpu)==null?void 0:z.requestAdapter()),e=await(t==null?void 0:t.requestDevice());if(!e){alert("need a browser that supports WebGPU");return}const o=navigator.gpu.getPreferredCanvasFormat(),a=h.getContext("webgpu");a.configure({device:e,format:o});const n=[];for(let i=0;i<w;++i)n.push({position:[0,1+i*.01],velocity:[0,-1],texCoord:[0,0],isObstacle:0});const r=new $(e,"object");await r.initialize(n);const s=[],b=.088;for(let i=1;i<=u;++i)if(i%2===1){s.push({position:[0,.9-b*(i-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1});for(let l=0;l<Math.floor(i/2);l++)s.push({position:[0+.08*(l+1),.9-b*(i-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1}),s.push({position:[0-.08*(l+1),.9-b*(i-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1})}else for(let l=0;l<Math.floor(i/2);l++)s.push({position:[.04+.08*l,.9-b*(i-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1}),s.push({position:[-.04-.08*l,.9-b*(i-1)],velocity:[0,0],texCoord:[0,0],isObstacle:1});const d=new $(e,"obstacle");await d.initialize(s);const U=e.createBuffer({label:"probabilities uniform buffer",size:w*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),G=e.createBuffer({label:"resultCriteria uniform buffer",size:u*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),W=s.slice(-u).map(i=>i.position[0]).sort((i,l)=>i-l);e.queue.writeBuffer(G,0,new Float32Array(W));const T=e.createBuffer({label:"result buffer",size:(u+1)*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),v=e.createBuffer({label:"result buffer",size:(u+1)*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST}),m=e.createBuffer({label:"constant uniform buffer",size:12*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});e.queue.writeBuffer(m,0,new Float32Array([1,0,0,.01,...O,.025,u,N,0,0]));const j=e.createBuffer({label:"time uniform buffer",size:2*Float32Array.BYTES_PER_ELEMENT,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),V=e.createComputePipeline({label:"compute movement pipeline",layout:"auto",compute:{module:e.createShaderModule({label:"movement compute shader",code:ie})}}),M=e.createComputePipeline({label:"compute result pipeline",layout:"auto",compute:{module:e.createShaderModule({label:"result compute shader",code:re})}}),_=e.createComputePipeline({label:"compute billboard pipeline",layout:"auto",compute:{module:e.createShaderModule({label:"billboard compute shader",code:ne})}}),L=e.createRenderPipeline({label:"main pipeline",layout:"auto",vertex:{module:e.createShaderModule({label:"main vertex shader",code:ae}),buffers:[{arrayStride:8*Float32Array.BYTES_PER_ELEMENT,attributes:[{shaderLocation:0,offset:0,format:"float32x2"},{shaderLocation:1,offset:2*Float32Array.BYTES_PER_ELEMENT,format:"float32x2"},{shaderLocation:2,offset:4*Float32Array.BYTES_PER_ELEMENT,format:"float32x2"},{shaderLocation:3,offset:6*Float32Array.BYTES_PER_ELEMENT,format:"float32"}]}]},fragment:{module:e.createShaderModule({label:"main fragment shader",code:se}),targets:[{format:o}]},primitive:{topology:"triangle-list"}}),H=e.createBindGroup({label:"compute movement bind group",layout:V.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:r.point}},{binding:1,resource:{buffer:d.point}},{binding:2,resource:{buffer:U}},{binding:3,resource:{buffer:m}},{binding:4,resource:{buffer:j}}]}),k=e.createBindGroup({label:"compute result bind group",layout:M.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:r.point}},{binding:1,resource:{buffer:m}},{binding:2,resource:{buffer:G}},{binding:3,resource:{buffer:T}}]}),K=e.createBindGroup({label:"compute billboard bind group",layout:_.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:r.point}},{binding:1,resource:{buffer:r.billboard}},{binding:2,resource:{buffer:m}}]}),X=e.createBindGroup({label:"compute obstacle billboard bind group",layout:_.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:d.point}},{binding:1,resource:{buffer:d.billboard}},{binding:2,resource:{buffer:m}}]}),J=e.createBindGroup({label:"main bind group",layout:L.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:m}}]}),A=e.createCommandEncoder({label:"encoder"}),C=A.beginComputePass({label:"compute billboard pass"});C.setPipeline(_),C.setBindGroup(0,X),C.dispatchWorkgroups(1,1),C.end();const Q=A.finish();e.queue.submit([Q]);let F=0;async function Y(){const i=performance.now(),l=i-F;F=i,e.queue.writeBuffer(j,0,new Float32Array([l])),e.queue.writeBuffer(U,0,le(w));const P=e.createCommandEncoder({label:"encoder"}),c=P.beginComputePass({label:"compute movement pass"});c.setPipeline(V),c.setBindGroup(0,H),c.dispatchWorkgroups(1,1),c.setPipeline(M),c.setBindGroup(0,k),c.dispatchWorkgroups(1,1),c.setPipeline(_),c.setBindGroup(0,K),c.dispatchWorkgroups(1,1),c.end();const p=P.beginRenderPass({label:"main pass",colorAttachments:[{view:a.getCurrentTexture().createView(),clearValue:[...q,1],loadOp:"clear",storeOp:"store"}]});p.setPipeline(L),p.setBindGroup(0,J),p.setVertexBuffer(0,d.billboard),p.draw(N*6),p.setVertexBuffer(0,r.billboard),p.draw(w*6),p.end(),P.copyBufferToBuffer(T,0,v,0,v.size);const Z=P.finish();e.queue.submit([Z]),await v.mapAsync(GPUMapMode.READ);const E=new Float32Array(v.getMappedRange().slice(0));v.unmap();const ee=Math.max.apply(null,Array.from(E));for(let g=0;g<u+1;g++)S[g].style.height=`${E[g]/ee*D}px`,E[g]&&(S[g].innerHTML=E[g].toString());requestAnimationFrame(Y)}Y()};ue();
